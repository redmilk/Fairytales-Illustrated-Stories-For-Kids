...
final class FirebaseClient: ImageDownloaderProvidable, UserSessionServiceProvidable {
    ...
    func makeFavoritesCategory() -> CategorySection {
        ...
        if let categories = categoriesInternalType.value {
            var favoriteStories: [StoryModel] = []
            categories.forEach { category in
                let favoritesInCategory = category.items.filter { $0.isFavorite }
                favoriteStories.append(contentsOf: favoritesInCategory)
            }
            favoritesSection.items.append(contentsOf: favoriteStories)
                    ...
                    .flatMap({ fairytaleDTO -> AnyPublisher<StoryModel, Never> in
                        Future<StoryModel, Never> ({ [weak self] promise in
                            ...
                            c = self.imageDownloader.fetchFromCache(path).sink(receiveCompletion: { completion in
                                ...
                                if let img = image {
                                    let isFavorite = self.userSession.checkIsStoryFavorite(with: fairytaleDTO.id_internal)
                                    let fairytale = StoryModel(dto: fairytaleDTO, isBoy: isForBoy, isFavorite: isFavorite)
